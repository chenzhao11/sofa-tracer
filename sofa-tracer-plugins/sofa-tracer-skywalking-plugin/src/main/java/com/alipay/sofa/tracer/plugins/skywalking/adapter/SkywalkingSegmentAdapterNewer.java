...
public class SkywalkingSegmentAdapterNewer {
    public Segment convertToSkywalkingSegment(SofaTracerSpan sofaTracerSpan){
        ...
        segment.addSpan(constructEntrySpan(sofaTracerSpan));
        Span exitSpan = constructExitSpan(sofaTracerSpan);
        if(exitSpan != null){
            segment.addSpan(exitSpan);
        }
        return segment;
    }
     ...
     * 构造EntrySpan
     */
    private Span constructEntrySpan(SofaTracerSpan sofaTracerSpan){
        Span entrySpan = new Span();

        entrySpan.setSpanId(0);
        entrySpan.setParentSpanId(-1);
        entrySpan.setStartTime(sofaTracerSpan.getStartTime());
        entrySpan.setEndTime(sofaTracerSpan.getEndTime());
        entrySpan.setOperationName(sofaTracerSpan.getOperationName());
        entrySpan.setSpanType(SpanType.Entry);
        //怎么判断是哪一层的？？
        entrySpan.setSpanLayer(SpanLayer.DB);
        entrySpan.setComponentId(
                ComponentName2ComponentId.componentName2IDMap.get(sofaTracerSpan.getSofaTracer().getTracerType()));
        //??
        entrySpan.setError(false);
        entrySpan.setSkipAnalysis(false);

        //转换tag
        entrySpan = convertSpanTags(sofaTracerSpan, entrySpan);
        //转换log
        convertSpanLogs(sofaTracerSpan, entrySpan);
        //添加ref
...
//        按照127.0.0.1:8080的格式组织的，目前也是还没有找到对应字段！！
        segmentReference .setNetworkAddressUsedAtPeer("127.0.0.1");
        entrySpan.addSegmentReference(segmentReference);
